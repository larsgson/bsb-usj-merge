export const before = {
  type: "USJ",
  version: "3.1",
  content: [
    {
      type: "book",
      marker: "id",
      code: "ROM",
      content: ["- Berean Study Bible"],
    },
    { type: "para", marker: "h", content: ["Romans"] },
    { type: "para", marker: "toc1", content: ["Romans"] },
    { type: "para", marker: "toc2", content: ["Romans"] },
    { type: "para", marker: "toc3", content: ["Romans"] },
    { type: "para", marker: "mt1", content: ["Romans"] },
    { type: "chapter", marker: "c", number: "1" },
    { type: "para", marker: "s1", content: ["Paul Greets the Saints in Rome"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "Paul, a servant of Christ Jesus, called to be an apostle, and set apart for the gospel of God—",
        { type: "verse", marker: "v", number: "2" },
        "the gospel He promised beforehand through His prophets in the Holy Scriptures,",
        { type: "verse", marker: "v", number: "3" },
        "regarding His Son, who was a descendant of David according to the flesh,",
        { type: "verse", marker: "v", number: "4" },
        "and who through the Spirit of holiness was declared with power to be the Son of God by His resurrection from the dead: Jesus Christ our Lord.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "5" },
        "Through Him and on behalf of His name, we received grace and apostleship to call all those among the Gentiles to the obedience that comes from faith.",
        { type: "verse", marker: "v", number: "6" },
        "And you also are among those who are called to belong to Jesus Christ.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "7" },
        "To all in Rome who are loved by God and called to be saints:",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        "Grace and peace to you from God our Father and the Lord Jesus Christ.",
      ],
    },
    { type: "para", marker: "s1", content: ["Unashamed of the Gospel"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "8" },
        "First, I thank my God through Jesus Christ for all of you, because your faith is being proclaimed all over the world.",
        { type: "verse", marker: "v", number: "9" },
        "God, whom I serve with my spirit",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["1:9"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                { type: "char", marker: "fq", content: ["in my spirit"] },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "in preaching the gospel of His Son, is my witness how constantly I remember you",
        { type: "verse", marker: "v", number: "10" },
        "in my prayers at all times, asking that now at last by God’s will I may succeed in coming to you.",
        { type: "verse", marker: "v", number: "11" },
        "For I long to see you so that I may impart to you some spiritual gift to strengthen you,",
        { type: "verse", marker: "v", number: "12" },
        "that is, that you and I may be mutually encouraged by each other’s faith.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "13" },
        "I do not want you to be unaware, brothers, how often I planned to come to you (but have been prevented from visiting",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["1:13"] },
            {
              type: "char",
              marker: "ft",
              content: [
                {
                  type: "char",
                  marker: "fqa",
                  content: ["but have been prevented"],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "until now), in order that I might have a harvest among you, just as I have had among the other Gentiles.",
        { type: "verse", marker: "v", number: "14" },
        "I am obligated both to Greeks and non-Greeks,",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["1:14"] },
            {
              type: "char",
              marker: "ft",
              content: [
                {
                  type: "char",
                  marker: "fqa",
                  content: ["to Greeks and barbarians"],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "both to the wise and the foolish.",
        { type: "verse", marker: "v", number: "15" },
        "That is why I am so eager to preach the gospel also to you who are in Rome.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "16" },
        "I am not ashamed of the gospel, because it is the power of God for salvation to everyone who believes, first to the Jew, then to the Greek.",
        { type: "verse", marker: "v", number: "17" },
        "For the gospel reveals the righteousness of God that comes by faith from start to finish,",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["1:17"] },
            {
              type: "char",
              marker: "ft",
              content: [
                {
                  type: "char",
                  marker: "fqa",
                  content: [
                    "For in it is revealed the righteousness of God from faith to faith",
                  ],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "just as it is written: “The righteous will live by faith.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["1:17"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Habakkuk 2:4"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    { type: "para", marker: "s1", content: ["God’s Wrath against Sin"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "18" },
        "The wrath of God is being revealed from heaven against all the godlessness and wickedness of men who suppress the truth by their wickedness.",
        { type: "verse", marker: "v", number: "19" },
        "For what may be known about God is plain to them, because God has made it plain to them.",
        { type: "verse", marker: "v", number: "20" },
        "For since the creation of the world God’s invisible qualities, His eternal power and divine nature, have been clearly seen, being understood from His workmanship, so that men are without excuse.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "21" },
        "For although they knew God, they neither glorified Him as God nor gave thanks to Him, but they became futile in their thinking and darkened in their foolish hearts.",
        { type: "verse", marker: "v", number: "22" },
        "Although they claimed to be wise, they became fools,",
        { type: "verse", marker: "v", number: "23" },
        "and exchanged the glory of the immortal God for images of mortal man and birds and animals and reptiles.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "24" },
        "Therefore God gave them over in the desires of their hearts to impurity for the dishonoring of their bodies with one another.",
        { type: "verse", marker: "v", number: "25" },
        "They exchanged the truth of God for a lie, and worshiped and served the creature rather than the Creator, who is forever worthy of praise!",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["1:25"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                { type: "char", marker: "fq", content: ["forever blessed"] },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "Amen.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "26" },
        "For this reason God gave them over to dishonorable passions. Even their women exchanged natural relations for unnatural ones.",
        { type: "verse", marker: "v", number: "27" },
        "Likewise, the men abandoned natural relations with women and burned with lust for one another. Men committed indecent acts with other men and received in themselves the due penalty for their error.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "28" },
        "Furthermore, since they did not see fit to acknowledge God, He gave them up to a depraved mind, to do what ought not to be done.",
        { type: "verse", marker: "v", number: "29" },
        "They have become filled with every kind of wickedness, evil, greed, and depravity. They are full of envy, murder, strife, deceit, and malice. They are gossips,",
        { type: "verse", marker: "v", number: "30" },
        "slanderers, God-haters, insolent, arrogant, and boastful. They invent new forms of evil; they disobey their parents.",
        { type: "verse", marker: "v", number: "31" },
        "They are senseless, faithless, heartless, merciless.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "32" },
        "Although they know God’s righteous decree that those who do such things are worthy of death, they not only continue to do these things, but also approve of those who practice them.",
      ],
    },
    { type: "chapter", marker: "c", number: "2" },
    { type: "para", marker: "s1", content: ["God’s Righteous Judgment"] },
    { type: "para", marker: "r", content: ["(Psalm 75:1–10)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "You, therefore, have no excuse, you who pass judgment on another. For on whatever grounds you judge the other, you are condemning yourself, because you who pass judgment do the same things.",
        { type: "verse", marker: "v", number: "2" },
        "And we know that God’s judgment against those who do such things is based on truth.",
        { type: "verse", marker: "v", number: "3" },
        "So when you, O man, pass judgment on others, yet do the same things, do you think you will escape God’s judgment?",
        { type: "verse", marker: "v", number: "4" },
        "Or do you disregard the riches of His kindness, tolerance, and patience, not realizing that God’s kindness leads you to repentance?",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "5" },
        "But because of your hard and unrepentant heart, you are storing up wrath against yourself for the day of wrath, when God’s righteous judgment will be revealed.",
        { type: "verse", marker: "v", number: "6" },
        "God “will repay each one according to his deeds.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["2:6"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Psalm 62:12"] },
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "7" },
        "To those who by perseverance in doing good seek glory, honor, and immortality, He will give eternal life.",
        { type: "verse", marker: "v", number: "8" },
        "But for those who are self-seeking and who reject the truth and follow wickedness, there will be wrath and anger.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "9" },
        "There will be trouble and distress for every human being who does evil, first for the Jew, then for the Greek;",
        { type: "verse", marker: "v", number: "10" },
        "but glory, honor, and peace for everyone who does good, first for the Jew, then for the Greek.",
        { type: "verse", marker: "v", number: "11" },
        "For God does not show favoritism.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "12" },
        "All who sin apart from the law will also perish apart from the law, and all who sin under the law will be judged by the law.",
        { type: "verse", marker: "v", number: "13" },
        "For it is not the hearers of the law who are righteous before God, but it is the doers of the law who will be declared righteous.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "14" },
        "Indeed, when Gentiles, who do not have the law, do by nature what the law requires, they are a law to themselves, even though they do not have the law.",
        { type: "verse", marker: "v", number: "15" },
        "So they show that the work of the law is written on their hearts, their consciences also bearing witness, and their thoughts either accusing or defending them",
        { type: "verse", marker: "v", number: "16" },
        "on the day when God will judge men’s secrets through Christ Jesus,",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["2:16"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "BYZ and TR ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["through Jesus Christ"],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "as proclaimed by my gospel.",
      ],
    },
    { type: "para", marker: "s1", content: ["The Jews and the Law"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "17" },
        "Now you, if you call yourself a Jew; if you rely on the law and boast in God;",
        { type: "verse", marker: "v", number: "18" },
        "if you know His will and approve of what is superior because you are instructed by the law;",
        { type: "verse", marker: "v", number: "19" },
        "if you are convinced that you are a guide for the blind, a light for those in darkness,",
        { type: "verse", marker: "v", number: "20" },
        "an instructor of the foolish, a teacher of infants, because you have in the law the embodiment of knowledge and truth—",
        { type: "verse", marker: "v", number: "21" },
        "you, then, who teach others, do you not teach yourself? You who preach against stealing, do you steal?",
        { type: "verse", marker: "v", number: "22" },
        "You who forbid adultery, do you commit adultery? You who abhor idols, do you rob temples?",
        { type: "verse", marker: "v", number: "23" },
        "You who boast in the law, do you dishonor God by breaking the law?",
        { type: "verse", marker: "v", number: "24" },
        "As it is written: “God’s name is blasphemed among the Gentiles because of you.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["2:24"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 52:5"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "25" },
        "Circumcision has value if you observe the law, but if you break the law, your circumcision has become uncircumcision.",
        { type: "verse", marker: "v", number: "26" },
        "If a man who is not circumcised keeps the requirements of the law, will not his uncircumcision be regarded as circumcision?",
        { type: "verse", marker: "v", number: "27" },
        "The one who is physically uncircumcised yet keeps the law will condemn you who, even though you have the written code and circumcision, are a lawbreaker.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "28" },
        "A man is not a Jew because he is one outwardly, nor is circumcision only outward and physical.",
        { type: "verse", marker: "v", number: "29" },
        "No, a man is a Jew because he is one inwardly, and circumcision is a matter of the heart, by the Spirit, not by the written code. Such a man’s praise does not come from men, but from God.",
      ],
    },
    { type: "chapter", marker: "c", number: "3" },
    { type: "para", marker: "s1", content: ["God Remains Faithful"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "What, then, is the advantage of being a Jew? Or what is the value of circumcision?",
        { type: "verse", marker: "v", number: "2" },
        "Much in every way. First of all, they have been entrusted with the very words",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["3:2"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                { type: "char", marker: "fq", content: ["the oracles"] },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "of God.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "3" },
        "What if some did not have faith? Will their lack of faith nullify God’s faithfulness?",
        { type: "verse", marker: "v", number: "4" },
        "Certainly not! Let God be true and every man a liar. As it is written:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“So that You may be proved right when You speak"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "and victorious when You judge.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["3:4"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["when You are judged"],
                },
                ";",
                { type: "char", marker: "xt", content: ["Psalm 51:4"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "5" },
        "But if our unrighteousness highlights the righteousness of God, what shall we say? That God is unjust to inflict His wrath on us? I am speaking in human terms.",
        { type: "verse", marker: "v", number: "6" },
        "Certainly not! In that case, how could God judge the world?",
        { type: "verse", marker: "v", number: "7" },
        "However, if my falsehood accentuates God’s truthfulness, to the increase of His glory, why am I still condemned as a sinner?",
        { type: "verse", marker: "v", number: "8" },
        "Why not say, as some slanderously claim that we say, “Let us do evil that good may result” ? Their condemnation is deserved!",
      ],
    },
    { type: "para", marker: "s1", content: ["There Is No One Righteous"] },
    {
      type: "para",
      marker: "r",
      content: ["(Psalm 14:1–7; Psalm 53:1–6; Isaiah 59:1–17)"],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "9" },
        "What then? Are we any better? Not at all. For we have already made the charge that Jews and Greeks alike are all under sin.",
        { type: "verse", marker: "v", number: "10" },
        "As it is written:",
      ],
    },
    { type: "para", marker: "q1", content: ["“There is no one righteous,"] },
    { type: "para", marker: "q2", content: ["not even one."] },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "11" },
        "There is no one who understands,",
      ],
    },
    { type: "para", marker: "q2", content: ["no one who seeks God."] },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "12" },
        "All have turned away,",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: ["they have together become worthless;"],
    },
    { type: "para", marker: "q1", content: ["there is no one who does good,"] },
    {
      type: "para",
      marker: "q2",
      content: [
        "not even one.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["3:12"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Psalm 14:1–3"] },
                "and",
                { type: "char", marker: "xt", content: ["Psalm 53:1–3"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "13" },
        "“Their throats are open graves;",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "their tongues practice deceit.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["3:13"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Psalm 5:9"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "“The venom of vipers is on their lips.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["3:13"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Psalm 140:3"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "14" },
        "“Their mouths are full",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "of cursing and bitterness.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["3:14"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Psalm 10:7"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "15" },
        "“Their feet are swift to shed blood;",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        { type: "verse", marker: "v", number: "16" },
        "ruin and misery lie in their wake,",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        { type: "verse", marker: "v", number: "17" },
        "and the way of peace they have not known.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["3:17"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 59:7–8"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "18" },
        "“There is no fear of God",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "before their eyes.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["3:18"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Psalm 36:1"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "19" },
        "Now we know that whatever the law says, it says to those who are under the law, so that every mouth may be silenced and the whole world held accountable to God.",
        { type: "verse", marker: "v", number: "20" },
        "Therefore no one will be justified in His sight by works of the law. For the law merely brings awareness of sin.",
      ],
    },
    {
      type: "para",
      marker: "s1",
      content: ["Righteousness through Faith in Christ"],
    },
    { type: "para", marker: "r", content: ["(Philippians 3:1–11)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "21" },
        "But now, apart from the law, the righteousness of God has been revealed, as attested by the Law and the Prophets.",
        { type: "verse", marker: "v", number: "22" },
        "And this righteousness from God comes through faith in Jesus Christ to all who believe. There is no distinction,",
        { type: "verse", marker: "v", number: "23" },
        "for all have sinned and fall short of the glory of God,",
        { type: "verse", marker: "v", number: "24" },
        "and are justified freely by His grace through the redemption that is in Christ Jesus.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "25" },
        "God presented Him as an atoning sacrifice",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["3:25"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                { type: "char", marker: "fq", content: ["as a propitiation"] },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "in His blood through faith, in order to demonstrate His righteousness, because in His forbearance He had passed over the sins committed beforehand.",
        { type: "verse", marker: "v", number: "26" },
        "He did this to demonstrate His righteousness at the present time, so as to be just and to justify the one who has faith in Jesus.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "27" },
        "Where, then, is boasting? It is excluded. On what principle? On that of works? No, but on that of faith.",
        { type: "verse", marker: "v", number: "28" },
        "For we maintain that a man is justified by faith apart from works of the law.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "29" },
        "Is God the God of Jews only? Is He not the God of Gentiles too? Yes, of Gentiles too,",
        { type: "verse", marker: "v", number: "30" },
        "since there is only one God, who will justify the circumcised by faith and the uncircumcised through that same faith.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "31" },
        "Do we, then, nullify the law by this faith? Certainly not! Instead, we uphold the law.",
      ],
    },
    { type: "chapter", marker: "c", number: "4" },
    { type: "para", marker: "s1", content: ["Abraham Justified by Faith"] },
    {
      type: "para",
      marker: "r",
      content: ["(Genesis 15:1–7; Psalm 32:1–11; Hebrews 11:8–19)"],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "What then shall we say that Abraham, our forefather according to the flesh, has discovered?",
        { type: "verse", marker: "v", number: "2" },
        "If Abraham was indeed justified by works, he had something to boast about, but not before God.",
        { type: "verse", marker: "v", number: "3" },
        "For what does the Scripture say? “Abraham believed God, and it was credited to him as righteousness.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["4:3"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Genesis 15:6"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "4" },
        "Now the wages of the worker are not credited as a gift, but as an obligation.",
        { type: "verse", marker: "v", number: "5" },
        "However, to the one who does not work, but believes in Him who justifies the ungodly, his faith is credited as righteousness.",
        { type: "verse", marker: "v", number: "6" },
        "And David speaks likewise of the blessedness of the man to whom God credits righteousness apart from works:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "7" },
        "“Blessed are they whose lawless acts are forgiven,",
      ],
    },
    { type: "para", marker: "q2", content: ["whose sins are covered."] },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "8" },
        "Blessed is the man",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "whose sin the Lord will never count against him.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["4:8"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Psalm 32:1–2"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "9" },
        "Is this blessing only on the circumcised, or also on the uncircumcised? We have been saying that Abraham’s faith was credited to him as righteousness.",
        { type: "verse", marker: "v", number: "10" },
        "In what context was it credited? Was it after his circumcision, or before? It was not after, but before.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["4:10"] },
            {
              type: "char",
              marker: "ft",
              content: [
                {
                  type: "char",
                  marker: "fqa",
                  content: [
                    "How then was it reckoned—being in circumcision, or in uncircumcision? Not in circumcision, but in uncircumcision.",
                  ],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "11" },
        "And he received the sign of circumcision as a seal of the righteousness that he had by faith while he was still uncircumcised. So then, he is the father of all who believe but are not circumcised, in order that righteousness might be credited to them.",
        { type: "verse", marker: "v", number: "12" },
        "And he is also the father of the circumcised who not only are circumcised, but who also walk in the footsteps of the faith that our father Abraham had before he was circumcised.",
      ],
    },
    { type: "para", marker: "s1", content: ["Abraham Receives the Promise"] },
    { type: "para", marker: "r", content: ["(Genesis 15:8–21)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "13" },
        "For the promise to Abraham and his offspring that he would be heir of the world was not given through the law, but through the righteousness that comes by faith.",
        { type: "verse", marker: "v", number: "14" },
        "For if those who live by the law are heirs, faith is useless and the promise is worthless,",
        { type: "verse", marker: "v", number: "15" },
        "because the law brings wrath. And where there is no law, there is no transgression.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "16" },
        "Therefore, the promise comes by faith, so that it may rest on grace and may be guaranteed to all Abraham’s offspring—not only to those who are of the law, but also to those who are of the faith of Abraham. He is the father of us all.",
        { type: "verse", marker: "v", number: "17" },
        "As it is written: “I have made you a father of many nations.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["4:17"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Genesis 17:5"] },
              ],
            },
          ],
          caller: "+",
        },
        "He is our father in the presence of God, in whom he believed, the God who gives life to the dead and calls into being what does not yet exist.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "18" },
        "Against all hope, Abraham in hope believed and so became the father of many nations, just as he had been told, “So shall your offspring be.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["4:18"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Genesis 15:5"] },
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "19" },
        "Without weakening in his faith, he acknowledged the decrepitness of his body (since he was about a hundred years old) and the lifelessness of Sarah’s womb.",
        { type: "verse", marker: "v", number: "20" },
        "Yet he did not waver through disbelief in the promise of God, but was strengthened in his faith and gave glory to God,",
        { type: "verse", marker: "v", number: "21" },
        "being fully persuaded that God was able to do what He had promised.",
        { type: "verse", marker: "v", number: "22" },
        "This is why “it was credited to him as righteousness.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["4:22"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Genesis 15:6"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "23" },
        "Now the words “it was credited to him” were written not only for Abraham,",
        { type: "verse", marker: "v", number: "24" },
        "but also for us, to whom righteousness will be credited—for us who believe in Him who raised Jesus our Lord from the dead.",
        { type: "verse", marker: "v", number: "25" },
        "He was delivered over to death for our trespasses and was raised to life for our justification.",
      ],
    },
    { type: "chapter", marker: "c", number: "5" },
    { type: "para", marker: "s1", content: ["The Triumph of Faith"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "Therefore, since we have been justified through faith, we have",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["5:1"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                { type: "char", marker: "fq", content: ["let us have"] },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "peace with God through our Lord Jesus Christ,",
        { type: "verse", marker: "v", number: "2" },
        "through whom we have gained access by faith into this grace in which we stand. And we rejoice",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["5:2"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                { type: "char", marker: "fq", content: ["exult"] },
                "; also in verses 3 and 11",
              ],
            },
          ],
          caller: "+",
        },
        "in the hope of the glory of God.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "3" },
        "Not only that, but we also rejoice in our sufferings, because we know that suffering produces perseverance;",
        { type: "verse", marker: "v", number: "4" },
        "perseverance, character; and character, hope.",
        { type: "verse", marker: "v", number: "5" },
        "And hope does not disappoint us, because God has poured out His love into our hearts through the Holy Spirit, whom He has given us.",
      ],
    },
    {
      type: "para",
      marker: "s1",
      content: ["Christ’s Sacrifice for the Ungodly"],
    },
    { type: "para", marker: "r", content: ["(John 3:1–21)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "6" },
        "For at just the right time, while we were still powerless, Christ died for the ungodly.",
        { type: "verse", marker: "v", number: "7" },
        "Very rarely will anyone die for a righteous man, though for a good man someone might possibly dare to die.",
        { type: "verse", marker: "v", number: "8" },
        "But God proves His love for us in this: While we were still sinners, Christ died for us.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "9" },
        "Therefore, since we have now been justified by His blood, how much more shall we be saved from wrath through Him!",
        { type: "verse", marker: "v", number: "10" },
        "For if, when we were enemies of God, we were reconciled to Him through the death of His Son, how much more, having been reconciled, shall we be saved through His life!",
        { type: "verse", marker: "v", number: "11" },
        "Not only that, but we also rejoice in God through our Lord Jesus Christ, through whom we have now received reconciliation.",
      ],
    },
    { type: "para", marker: "s1", content: ["Death in Adam, Life in Christ"] },
    { type: "para", marker: "r", content: ["(Genesis 3:1–7)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "12" },
        "Therefore, just as sin entered the world through one man, and death through sin, so also death was passed on to all men, because all sinned.",
        { type: "verse", marker: "v", number: "13" },
        "For sin was in the world before the law was given; but sin is not taken into account when there is no law.",
        { type: "verse", marker: "v", number: "14" },
        "Nevertheless, death reigned from Adam until Moses, even over those who did not sin in the way that Adam transgressed. He is a pattern of the One to come.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "15" },
        "But the gift is not like the trespass. For if the many died by the trespass of the one man, how much more did God’s grace and the gift that came by the grace of the one man, Jesus Christ, abound to the many!",
        { type: "verse", marker: "v", number: "16" },
        "Again, the gift is not like the result of the one man’s sin: The judgment that followed one sin brought condemnation, but the gift that followed many trespasses brought justification.",
        { type: "verse", marker: "v", number: "17" },
        "For if, by the trespass of the one man, death reigned through that one man, how much more will those who receive the abundance of grace and of the gift of righteousness reign in life through the one man, Jesus Christ!",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "18" },
        "So then, just as one trespass brought condemnation for all men, so also one act of righteousness brought justification and life for all men.",
        { type: "verse", marker: "v", number: "19" },
        "For just as through the disobedience of the one man the many were made sinners, so also through the obedience of the one man the many will be made righteous.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "20" },
        "The law came in so that the trespass would increase; but where sin increased, grace increased all the more,",
        { type: "verse", marker: "v", number: "21" },
        "so that, just as sin reigned in death, so also grace might reign through righteousness to bring eternal life through Jesus Christ our Lord.",
      ],
    },
    { type: "chapter", marker: "c", number: "6" },
    { type: "para", marker: "s1", content: ["Dead to Sin, Alive to God"] },
    { type: "para", marker: "r", content: ["(2 Corinthians 4:7–18)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "What then shall we say? Shall we continue in sin so that grace may increase?",
        { type: "verse", marker: "v", number: "2" },
        "Certainly not! How can we who died to sin live in it any longer?",
        { type: "verse", marker: "v", number: "3" },
        "Or aren’t you aware that all of us who were baptized into Christ Jesus were baptized into His death?",
        { type: "verse", marker: "v", number: "4" },
        "We were therefore buried with Him through baptism into death, in order that, just as Christ was raised from the dead through the glory of the Father, we too may walk in newness of life.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "5" },
        "For if we have been united with Him like this in His death, we will certainly also be united with Him in His resurrection.",
        { type: "verse", marker: "v", number: "6" },
        "We know that our old self was crucified with Him so that the body of sin might be rendered powerless, that we should no longer be slaves to sin.",
        { type: "verse", marker: "v", number: "7" },
        "For anyone who has died has been freed from sin.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "8" },
        "Now if we died with Christ, we believe that we will also live with Him.",
        { type: "verse", marker: "v", number: "9" },
        "For we know that since Christ was raised from the dead, He cannot die again; death no longer has dominion over Him.",
        { type: "verse", marker: "v", number: "10" },
        "The death He died, He died to sin once for all; but the life He lives, He lives to God.",
        { type: "verse", marker: "v", number: "11" },
        "So you too must count yourselves dead to sin, but alive to God in Christ Jesus.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "12" },
        "Therefore do not let sin reign in your mortal body so that you obey its desires.",
        { type: "verse", marker: "v", number: "13" },
        "Do not present the parts of your body to sin as instruments of wickedness, but present yourselves to God as those who have been brought from death to life; and present the parts of your body to Him as instruments of righteousness.",
        { type: "verse", marker: "v", number: "14" },
        "For sin shall not be your master, because you are not under law, but under grace.",
      ],
    },
    { type: "para", marker: "s1", content: ["The Wages of Sin"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "15" },
        "What then? Shall we sin because we are not under law, but under grace? Certainly not!",
        { type: "verse", marker: "v", number: "16" },
        "Do you not know that when you offer yourselves as obedient slaves, you are slaves to the one you obey, whether you are slaves to sin leading to death, or to obedience leading to righteousness?",
        { type: "verse", marker: "v", number: "17" },
        "But thanks be to God that, though you once were slaves to sin, you wholeheartedly obeyed the form of teaching to which you were committed.",
        { type: "verse", marker: "v", number: "18" },
        "You have been set free from sin and have become slaves to righteousness.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "19" },
        "I am speaking in human terms because of the weakness of your flesh. Just as you used to offer the parts of your body in slavery to impurity and to escalating wickedness, so now offer them in slavery to righteousness leading to holiness.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "20" },
        "For when you were slaves to sin, you were free of obligation to righteousness.",
        { type: "verse", marker: "v", number: "21" },
        "What fruit did you reap at that time from the things of which you are now ashamed? The outcome of those things is death.",
        { type: "verse", marker: "v", number: "22" },
        "But now that you have been set free from sin and have become slaves to God, the fruit you reap leads to holiness, and the outcome is eternal life.",
        { type: "verse", marker: "v", number: "23" },
        "For the wages of sin is death, but the gift of God is eternal life in Christ Jesus our Lord.",
      ],
    },
    { type: "chapter", marker: "c", number: "7" },
    { type: "para", marker: "s1", content: ["Release from the Law"] },
    { type: "para", marker: "r", content: ["(Galatians 3:15–25)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "Do you not know, brothers (for I am speaking to those who know the law), that the law has authority over a man only as long as he lives?",
        { type: "verse", marker: "v", number: "2" },
        "For instance, a married woman is bound by law to her husband as long as he lives. But if her husband dies, she is released from the law of marriage.",
        { type: "verse", marker: "v", number: "3" },
        "So then, if she is joined to another man while her husband is still alive, she is called an adulteress; but if her husband dies, she is free from that law and is not an adulteress if she marries another man.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "4" },
        "Therefore, my brothers, you also died to the law through the body of Christ, that you might belong to another, to Him who was raised from the dead, in order that we might bear fruit to God.",
        { type: "verse", marker: "v", number: "5" },
        "For when we lived according to the flesh, the sinful passions aroused by the law were at work in our bodies, bearing fruit for death.",
        { type: "verse", marker: "v", number: "6" },
        "But now, having died to what bound us, we have been released from the law, so that we serve in the new way of the Spirit, and not in the old way of the written code.",
      ],
    },
    { type: "para", marker: "s1", content: ["God’s Law Is Holy"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "7" },
        "What then shall we say? Is the law sin? Certainly not! Indeed, I would not have been mindful of sin if not for the law. For I would not have been aware of coveting if the law had not said, “Do not covet.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["7:7"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Exodus 20:17"] },
                ";",
                { type: "char", marker: "xt", content: ["Deuteronomy 5:21"] },
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "8" },
        "But sin, seizing its opportunity through the commandment, produced in me every kind of covetous desire. For apart from the law, sin is dead.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "9" },
        "Once I was alive apart from the law; but when the commandment came, sin sprang to life and I died.",
        { type: "verse", marker: "v", number: "10" },
        "So I discovered that the very commandment that was meant to bring life actually brought death.",
        { type: "verse", marker: "v", number: "11" },
        "For sin, seizing its opportunity through the commandment, deceived me and through the commandment put me to death.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "12" },
        "So then, the law is holy, and the commandment is holy, righteous, and good.",
      ],
    },
    { type: "para", marker: "s1", content: ["Struggling with Sin"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "13" },
        "Did that which is good, then, become death to me? Certainly not! But in order that sin might be exposed as sin, it produced death in me through what was good, so that through the commandment sin might become utterly sinful.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "14" },
        "We know that the law is spiritual; but I am unspiritual, sold as a slave to sin.",
        { type: "verse", marker: "v", number: "15" },
        "I do not understand what I do. For what I want to do, I do not do. But what I hate, I do.",
        { type: "verse", marker: "v", number: "16" },
        "And if I do what I do not want to do, I admit that the law is good.",
        { type: "verse", marker: "v", number: "17" },
        "In that case, it is no longer I who do it, but it is sin living in me that does it.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "18" },
        "I know that nothing good lives in me, that is, in my flesh; for I have the desire to do what is good, but I cannot carry it out.",
        { type: "verse", marker: "v", number: "19" },
        "For I do not do the good I want to do. Instead, I keep on doing the evil I do not want to do.",
        { type: "verse", marker: "v", number: "20" },
        "And if I do what I do not want, it is no longer I who do it, but it is sin living in me that does it.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "21" },
        "So this is the principle I have discovered: When I want to do good, evil is right there with me.",
        { type: "verse", marker: "v", number: "22" },
        "For in my inner being I delight in God’s law.",
        { type: "verse", marker: "v", number: "23" },
        "But I see another law at work in my body, warring against the law of my mind and holding me captive to the law of sin that dwells within me.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["7:23"] },
            {
              type: "char",
              marker: "ft",
              content: [
                {
                  type: "char",
                  marker: "fqa",
                  content: ["captive to the law of sin being in my members."],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "24" },
        "What a wretched man I am! Who will rescue me from this body of death?",
        { type: "verse", marker: "v", number: "25" },
        "Thanks be to God, through Jesus Christ our Lord!",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        "So then, with my mind I serve the law of God, but with my flesh I serve the law of sin.",
      ],
    },
    { type: "chapter", marker: "c", number: "8" },
    { type: "para", marker: "s1", content: ["Walking by the Spirit"] },
    {
      type: "para",
      marker: "r",
      content: ["(Ezekiel 36:16–38; Galatians 5:16–26)"],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "Therefore, there is now no condemnation for those who are in Christ Jesus.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["8:1"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "BYZ and TR ",
                {
                  type: "char",
                  marker: "fq",
                  content: [
                    "in Christ Jesus, who do not walk according to the flesh but according to the Spirit.",
                  ],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "2" },
        "For in Christ Jesus the law of the Spirit of life set you",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["8:2"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "BYZ and TR ",
                { type: "char", marker: "fq", content: ["me"] },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "free from the law of sin and death.",
        { type: "verse", marker: "v", number: "3" },
        "For what the law was powerless to do in that it was weakened by the flesh, God did by sending His own Son in the likeness of sinful man, as an offering for sin.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["8:3"] },
            {
              type: "char",
              marker: "ft",
              content: [
                {
                  type: "char",
                  marker: "fqa",
                  content: ["in the likeness of sinful flesh and for sin"],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "He thus condemned sin in the flesh,",
        { type: "verse", marker: "v", number: "4" },
        "so that the righteous standard of the law might be fulfilled in us, who do not walk according to the flesh but according to the Spirit.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "5" },
        "Those who live according to the flesh set their minds on the things of the flesh; but those who live according to the Spirit set their minds on the things of the Spirit.",
        { type: "verse", marker: "v", number: "6" },
        "The mind of the flesh is death, but the mind of the Spirit is life and peace,",
        { type: "verse", marker: "v", number: "7" },
        "because the mind of the flesh is hostile to God: It does not submit to God’s law, nor can it do so.",
        { type: "verse", marker: "v", number: "8" },
        "Those controlled by the flesh",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["8:8"] },
            {
              type: "char",
              marker: "ft",
              content: [
                {
                  type: "char",
                  marker: "fqa",
                  content: ["Those being in the flesh"],
                },
                "; similarly in verse 9",
              ],
            },
          ],
          caller: "+",
        },
        "cannot please God.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "9" },
        "You, however, are controlled not by the flesh, but by the Spirit, if the Spirit of God lives in you. And if anyone does not have the Spirit of Christ, he does not belong to Christ.",
        { type: "verse", marker: "v", number: "10" },
        "But if Christ is in you, your body is dead because of sin, yet your spirit is alive",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["8:10"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["yet the Spirit is life"],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "because of righteousness.",
        { type: "verse", marker: "v", number: "11" },
        "And if the Spirit of Him who raised Jesus from the dead is living in you, He who raised Christ Jesus from the dead",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["8:11"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "NA, BYZ, and TR ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["raised Christ from the dead"],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "will also give life to your mortal bodies through His Spirit, who lives in you.",
      ],
    },
    { type: "para", marker: "s1", content: ["Heirs with Christ"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "12" },
        "Therefore, brothers, we have an obligation, but it is not to the flesh, to live according to it.",
        { type: "verse", marker: "v", number: "13" },
        "For if you live according to the flesh, you will die; but if by the Spirit you put to death the deeds of the body, you will live.",
        { type: "verse", marker: "v", number: "14" },
        "For all who are led by the Spirit of God are sons of God.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "15" },
        "For you did not receive a spirit of slavery that returns you to fear, but you received the Spirit of adoption to sonship, by whom we cry, “Abba! Father!”",
        { type: "verse", marker: "v", number: "16" },
        "The Spirit Himself testifies with our spirit that we are God’s children.",
        { type: "verse", marker: "v", number: "17" },
        "And if we are children, then we are heirs: heirs of God and co-heirs with Christ—if indeed we suffer with Him, so that we may also be glorified with Him.",
      ],
    },
    { type: "para", marker: "s1", content: ["Future Glory"] },
    { type: "para", marker: "r", content: ["(2 Corinthians 5:1–10)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "18" },
        "I consider that our present sufferings are not comparable to the glory that will be revealed in us.",
        { type: "verse", marker: "v", number: "19" },
        "The creation waits in eager expectation for the revelation of the sons of God.",
        { type: "verse", marker: "v", number: "20" },
        "For the creation was subjected to futility, not by its own will, but because of the One who subjected it, in hope",
        { type: "verse", marker: "v", number: "21" },
        "that the creation itself will be set free from its bondage to decay and brought into the glorious freedom of the children of God.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "22" },
        "We know that the whole creation has been groaning together in the pains of childbirth until the present time.",
        { type: "verse", marker: "v", number: "23" },
        "Not only that, but we ourselves, who have the firstfruits of the Spirit, groan inwardly as we wait eagerly for our adoption as sons, the redemption of our bodies.",
        { type: "verse", marker: "v", number: "24" },
        "For in this hope we were saved; but hope that is seen is no hope at all. Who hopes for what he can already see?",
        { type: "verse", marker: "v", number: "25" },
        "But if we hope for what we do not yet see, we wait for it patiently.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "26" },
        "In the same way, the Spirit helps us in our weakness. For we do not know how we ought to pray, but the Spirit Himself intercedes for us with groans too deep for words.",
        { type: "verse", marker: "v", number: "27" },
        "And He who searches our hearts knows the mind of the Spirit, because the Spirit intercedes for the saints according to the will of God.",
      ],
    },
    { type: "para", marker: "s1", content: ["God Works in All Things"] },
    { type: "para", marker: "r", content: ["(Ephesians 1:3–14)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "28" },
        "And we know that God works all things together for the good of those who love Him, who are called according to His purpose.",
        { type: "verse", marker: "v", number: "29" },
        "For those God foreknew, He also predestined to be conformed to the image of His Son, so that He would be the firstborn among many brothers.",
        { type: "verse", marker: "v", number: "30" },
        "And those He predestined, He also called; those He called, He also justified; those He justified, He also glorified.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "31" },
        "What then shall we say in response to these things? If God is for us, who can be against us?",
        { type: "verse", marker: "v", number: "32" },
        "He who did not spare His own Son but gave Him up for us all, how will He not also, along with Him, freely give us all things?",
        { type: "verse", marker: "v", number: "33" },
        "Who will bring any charge against God’s elect? It is God who justifies.",
        { type: "verse", marker: "v", number: "34" },
        "Who is there to condemn us? For Christ Jesus, who died, and more than that was raised to life, is at the right hand of God—and He is interceding for us.",
      ],
    },
    { type: "para", marker: "s1", content: ["More than Conquerors"] },
    { type: "para", marker: "r", content: ["(Psalm 44:1–26)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "35" },
        "Who shall separate us from the love of Christ? Shall trouble or distress or persecution or famine or nakedness or danger or sword?",
        { type: "verse", marker: "v", number: "36" },
        "As it is written:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“For Your sake we face death all day long;"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "we are considered as sheep to be slaughtered.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["8:36"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Psalm 44:22"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "37" },
        "No, in all these things we are more than conquerors through Him who loved us.",
        { type: "verse", marker: "v", number: "38" },
        "For I am convinced that neither death nor life, neither angels nor principalities, neither the present nor the future, nor any powers,",
        { type: "verse", marker: "v", number: "39" },
        "neither height nor depth, nor anything else in all creation, will be able to separate us from the love of God that is in Christ Jesus our Lord.",
      ],
    },
    { type: "chapter", marker: "c", number: "9" },
    { type: "para", marker: "s1", content: ["Paul’s Concern for the Jews"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "I speak the truth in Christ; I am not lying, as confirmed by my conscience in the Holy Spirit.",
        { type: "verse", marker: "v", number: "2" },
        "I have deep sorrow and unceasing anguish in my heart.",
        { type: "verse", marker: "v", number: "3" },
        "For I could wish that I myself were cursed and cut off from Christ for the sake of my brothers, my own flesh and blood,",
        { type: "verse", marker: "v", number: "4" },
        "the people of Israel. Theirs is the adoption as sons; theirs the divine glory and the covenants; theirs the giving of the law, the temple worship, and the promises.",
        { type: "verse", marker: "v", number: "5" },
        "Theirs are the patriarchs, and from them proceeds the human descent of Christ, who is God over all, forever worthy of praise!",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:5"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                { type: "char", marker: "fq", content: ["forever blessed"] },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "Amen.",
      ],
    },
    { type: "para", marker: "s1", content: ["God’s Sovereign Choice"] },
    {
      type: "para",
      marker: "r",
      content: ["(Genesis 25:19–28; Malachi 1:1–5)"],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "6" },
        "It is not as though God’s word has failed. For not all who are descended from Israel are Israel.",
        { type: "verse", marker: "v", number: "7" },
        "Nor because they are Abraham’s descendants are they all his children. On the contrary, “Through Isaac your offspring will be reckoned.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:7"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Genesis 21:12"] },
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "8" },
        "So it is not the children of the flesh who are God’s children, but it is the children of the promise who are regarded as offspring.",
        { type: "verse", marker: "v", number: "9" },
        "For this is what the promise stated: “At the appointed time I will return, and Sarah will have a son.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:9"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Genesis 18:14"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "10" },
        "Not only that, but Rebecca’s children were conceived by one man, our father Isaac.",
        { type: "verse", marker: "v", number: "11" },
        "Yet before the twins were born or had done anything good or bad, in order that God’s plan of election might stand,",
        { type: "verse", marker: "v", number: "12" },
        "not by works but by Him who calls, she was told, “The older will serve the younger.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:12"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Genesis 25:23"] },
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "13" },
        "So it is written: “Jacob I loved, but Esau I hated.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:13"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Malachi 1:2–3"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "14" },
        "What then shall we say? Is God unjust? Certainly not!",
        { type: "verse", marker: "v", number: "15" },
        "For He says to Moses:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“I will have mercy on whom I have mercy,"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "and I will have compassion on whom I have compassion.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:15"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Exodus 33:19"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "16" },
        "So then, it does not depend on man’s desire or effort, but on God’s mercy.",
        { type: "verse", marker: "v", number: "17" },
        "For the Scripture says to Pharaoh: “I raised you up for this very purpose, that I might display My power in you, and that My name might be proclaimed in all the earth.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:17"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Exodus 9:16"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "18" },
        "Therefore God has mercy on whom He wants to have mercy, and He hardens whom He wants to harden.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "19" },
        "One of you will say to me, “Then why does God still find fault? For who can resist His will?”",
        { type: "verse", marker: "v", number: "20" },
        "But who are you, O man, to talk back to God? Shall what is formed say to Him who formed it, “Why did You make me like this?”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:20"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 29:16"] },
                ";",
                { type: "char", marker: "xt", content: ["Isaiah 45:9"] },
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "21" },
        "Does not the potter have the right to make from the same lump of clay one vessel for special occasions and another for common use?",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "22" },
        "What if God, intending to show His wrath and make His power known, bore with great patience the vessels of His wrath, prepared for destruction?",
        { type: "verse", marker: "v", number: "23" },
        "What if He did this to make the riches of His glory known to the vessels of His mercy, whom He prepared in advance for glory—",
        { type: "verse", marker: "v", number: "24" },
        "including us, whom He has called not only from the Jews, but also from the Gentiles?",
        { type: "verse", marker: "v", number: "25" },
        "As He says in Hosea:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“I will call them ‘My People’ who are not My people,"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "and I will call her ‘My Beloved’ who is not My beloved,”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:25"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Hosea 2:23"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [{ type: "verse", marker: "v", number: "26" }, "and,"],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        "“It will happen that in the very place where it was said to them,",
      ],
    },
    { type: "para", marker: "q2", content: ["‘You are not My people,’"] },
    { type: "para", marker: "q1", content: ["they will be called"] },
    {
      type: "para",
      marker: "q2",
      content: [
        "‘sons of the living God.’”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:26"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Hosea 1:10"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "27" },
        "Isaiah cries out concerning Israel:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        "“Though the number of the Israelites is like the sand of the sea,",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: ["only the remnant will be saved."],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "28" },
        "For the Lord will carry out His sentence on the earth",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "thoroughly and decisively.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:28"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 10:22–23"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "29" },
        "It is just as Isaiah foretold:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“Unless the Lord of Hosts had left us descendants,"],
    },
    {
      type: "para",
      marker: "q2",
      content: ["we would have become like Sodom,"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "we would have resembled Gomorrah.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:29"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 1:9"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    { type: "para", marker: "s1", content: ["Israel’s Unbelief"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "30" },
        "What then will we say? That the Gentiles, who did not pursue righteousness, have obtained it, a righteousness that is by faith;",
        { type: "verse", marker: "v", number: "31" },
        "but Israel, who pursued a law of righteousness, has not attained it.",
        { type: "verse", marker: "v", number: "32" },
        "Why not? Because their pursuit was not by faith, but as if it were by works. They stumbled over the stumbling stone,",
        { type: "verse", marker: "v", number: "33" },
        "as it is written:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“See, I lay in Zion a stone of stumbling"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "and a rock of offense;",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:33"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 8:14"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["and the one who believes in Him"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "will never be put to shame.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["9:33"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 28:16"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    { type: "chapter", marker: "c", number: "10" },
    { type: "para", marker: "s1", content: ["The Word Brings Salvation"] },
    { type: "para", marker: "r", content: ["(Isaiah 65:1–16)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "Brothers, my heart’s desire and prayer to God for the Israelites is for their salvation.",
        { type: "verse", marker: "v", number: "2" },
        "For I testify about them that they are zealous for God, but not on the basis of knowledge.",
        { type: "verse", marker: "v", number: "3" },
        "Because they were ignorant of God’s righteousness and sought to establish their own, they did not submit to God’s righteousness.",
        { type: "verse", marker: "v", number: "4" },
        "For Christ is the end of the law, to bring righteousness to everyone who believes.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "5" },
        "For concerning the righteousness that is by the law, Moses writes: “The man who does these things will live by them.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["10:5"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Leviticus 18:5"] },
                "; see also",
                {
                  type: "char",
                  marker: "xt",
                  content: ["Ezekiel 20:11, 13, and 21"],
                },
                ".",
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "6" },
        "But the righteousness that is by faith says: “Do not say in your heart, ‘Who will ascend into heaven?’",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["10:6"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Deuteronomy 30:12"] },
              ],
            },
          ],
          caller: "+",
        },
        "(that is, to bring Christ down)",
        { type: "verse", marker: "v", number: "7" },
        "or, ‘Who will descend into the Abyss?’",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["10:7"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "See",
                { type: "char", marker: "xt", content: ["Deuteronomy 30:13"] },
                ".",
              ],
            },
          ],
          caller: "+",
        },
        "(that is, to bring Christ up from the dead).”",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "8" },
        "But what does it say? “The word is near you; it is in your mouth and in your heart,”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["10:8"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Deuteronomy 30:14"] },
              ],
            },
          ],
          caller: "+",
        },
        "that is, the word of faith we are proclaiming:",
        { type: "verse", marker: "v", number: "9" },
        "that if you confess with your mouth, “Jesus is Lord,” and believe in your heart that God raised Him from the dead, you will be saved.",
        { type: "verse", marker: "v", number: "10" },
        "For with your heart you believe and are justified, and with your mouth you confess and are saved.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "11" },
        "It is just as the Scripture says: “Anyone who believes in Him will never be put to shame.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["10:11"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 28:16"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "12" },
        "For there is no difference between Jew and Greek: The same Lord is Lord of all, and gives richly to all who call on Him,",
        { type: "verse", marker: "v", number: "13" },
        "for, “Everyone who calls on the name of the Lord will be saved.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["10:13"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Joel 2:32"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "14" },
        "How then can they call on the One in whom they have not believed? And how can they believe in the One of whom they have not heard? And how can they hear without someone to preach?",
        { type: "verse", marker: "v", number: "15" },
        "And how can they preach unless they are sent? As it is written: “How beautiful are the feet of those who bring good news!”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["10:15"] },
            {
              type: "char",
              marker: "ft",
              content: [
                {
                  type: "char",
                  marker: "fqa",
                  content: ["good news of good things"],
                },
                "; BYZ and TR ",
                {
                  type: "char",
                  marker: "fq",
                  content: [
                    "How beautiful are the feet of those who bring good news of peace, who bring good news of good things",
                  ],
                },
                ";",
                { type: "char", marker: "xt", content: ["Isaiah 52:7"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "16" },
        "But not all of them welcomed the good news. For Isaiah says, “Lord, who has believed our message?”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["10:16"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 53:1"] },
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "17" },
        "Consequently, faith comes by hearing, and hearing by the word of Christ.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "18" },
        "But I ask, did they not hear? Indeed they did:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“Their voice has gone out into all the earth,"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "their words to the ends of the world.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["10:18"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Psalm 19:4"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "19" },
        "I ask instead, did Israel not understand? First, Moses says:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“I will make you jealous by those who are not a nation;"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "I will make you angry by a nation without understanding.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["10:19"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Deuteronomy 32:21"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "20" },
        "And Isaiah boldly says:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“I was found by those who did not seek Me;"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "I revealed Myself to those who did not ask for Me.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["10:20"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 65:1"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "21" },
        "But as for Israel he says:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“All day long I have held out My hands"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "to a disobedient and obstinate people.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["10:21"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 65:2"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    { type: "chapter", marker: "c", number: "11" },
    { type: "para", marker: "s1", content: ["A Remnant Chosen by Grace"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "I ask then, did God reject His people? Certainly not! I am an Israelite myself, a descendant of Abraham, from the tribe of Benjamin.",
        { type: "verse", marker: "v", number: "2" },
        "God did not reject His people, whom He foreknew. Do you not know what the Scripture says about Elijah, how he appealed to God against Israel:",
        { type: "verse", marker: "v", number: "3" },
        "“Lord, they have killed Your prophets and torn down Your altars. I am the only one left, and they are seeking my life as well”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["11:3"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["1 Kings 19:10, 14"] },
              ],
            },
          ],
          caller: "+",
        },
        "?",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "4" },
        "And what was the divine reply to him? “I have reserved for Myself seven thousand men who have not bowed the knee to Baal.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["11:4"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["1 Kings 19:18"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "5" },
        "In the same way, at the present time there is a remnant chosen by grace.",
        { type: "verse", marker: "v", number: "6" },
        "And if it is by grace, then it is no longer by works. Otherwise, grace would no longer be grace.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["11:6"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "BYZ and TR include ",
                {
                  type: "char",
                  marker: "fq",
                  content: [
                    "But if it is by works, then it is no longer grace; otherwise work is no longer work.",
                  ],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "7" },
        "What then? What Israel was seeking, it failed to obtain, but the elect did. The others were hardened,",
        { type: "verse", marker: "v", number: "8" },
        "as it is written:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“God gave them a spirit of stupor,"],
    },
    { type: "para", marker: "q2", content: ["eyes that could not see,"] },
    { type: "para", marker: "q2", content: ["and ears that could not hear,"] },
    {
      type: "para",
      marker: "q1",
      content: [
        "to this very day.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["11:8"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "See",
                { type: "char", marker: "xt", content: ["Deuteronomy 29:4"] },
                "and",
                { type: "char", marker: "xt", content: ["Isaiah 29:10"] },
                ".",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [{ type: "verse", marker: "v", number: "9" }, "And David says:"],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“May their table become a snare and a trap,"],
    },
    {
      type: "para",
      marker: "q2",
      content: ["a stumbling block and a retribution to them."],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "10" },
        "May their eyes be darkened so they cannot see,",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "and their backs be bent forever.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["11:10"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Psalm 69:22–23"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    { type: "para", marker: "s1", content: ["The Ingrafting of the Gentiles"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "11" },
        "I ask then, did they stumble so as to fall beyond recovery?",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["11:11"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["did they stumble so as to lose their share?"],
                },
                " Literally ",
                {
                  type: "char",
                  marker: "fqa",
                  content: ["did they stumble that they might fall?"],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "Certainly not! However, because of their trespass, salvation has come to the Gentiles to make Israel jealous.",
        { type: "verse", marker: "v", number: "12" },
        "But if their trespass means riches for the world, and their failure means riches for the Gentiles, how much greater riches will their fullness bring!",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "13" },
        "I am speaking to you Gentiles. Inasmuch as I am the apostle to the Gentiles, I magnify my ministry",
        { type: "verse", marker: "v", number: "14" },
        "in the hope that I may provoke my own people to jealousy and save some of them.",
        { type: "verse", marker: "v", number: "15" },
        "For if their rejection is the reconciliation of the world, what will their acceptance be but life from the dead?",
        { type: "verse", marker: "v", number: "16" },
        "If the first part of the dough is holy, so is the whole batch; if the root is holy, so are the branches.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "17" },
        "Now if some branches have been broken off, and you, a wild olive shoot, have been grafted in among the others to share in the nourishment of the olive root,",
        { type: "verse", marker: "v", number: "18" },
        "do not boast over those branches. If you do, remember this: You do not support the root, but the root supports you.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "19" },
        "You will say then, “Branches were broken off so that I could be grafted in.”",
        { type: "verse", marker: "v", number: "20" },
        "That is correct: They were broken off because of unbelief, but you stand by faith. Do not be arrogant, but be afraid.",
        { type: "verse", marker: "v", number: "21" },
        "For if God did not spare the natural branches, He will certainly not",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["11:21"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["He will perhaps not"],
                },
                "; SBL, NE, and WH ",
                { type: "char", marker: "fq", content: ["He will not"] },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "spare you either.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "22" },
        "Take notice, therefore, of the kindness and severity of God: severity to those who fell, but kindness to you, if you continue in His kindness. Otherwise you also will be cut off.",
        { type: "verse", marker: "v", number: "23" },
        "And if they do not persist in unbelief, they will be grafted in, for God is able to graft them in again.",
        { type: "verse", marker: "v", number: "24" },
        "For if you were cut from a wild olive tree, and contrary to nature were grafted into one that is cultivated, how much more readily will these, the natural branches, be grafted into their own olive tree!",
      ],
    },
    { type: "para", marker: "s1", content: ["All Israel Will Be Saved"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "25" },
        "I do not want you to be ignorant of this mystery, brothers, so that you will not be conceited: A hardening in part has come to Israel, until the full number of the Gentiles has come in.",
        { type: "verse", marker: "v", number: "26" },
        "And so all Israel will be saved, as it is written:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“The Deliverer will come from Zion;"],
    },
    {
      type: "para",
      marker: "q2",
      content: ["He will remove godlessness from Jacob."],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "27" },
        "And this is My covenant with them",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "when I take away their sins.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["11:27"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 27:9"] },
                "and",
                { type: "char", marker: "xt", content: ["Isaiah 59:20–21"] },
                "; see also LXX",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "28" },
        "Regarding the gospel, they are enemies on your account; but regarding election, they are loved on account of the patriarchs.",
        { type: "verse", marker: "v", number: "29" },
        "For God’s gifts and His call are irrevocable.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "30" },
        "Just as you who formerly disobeyed God have now received mercy through their disobedience,",
        { type: "verse", marker: "v", number: "31" },
        "so they too have now disobeyed, in order that they too may now receive mercy through the mercy shown to you.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["11:31"] },
            {
              type: "char",
              marker: "ft",
              content: [
                {
                  type: "char",
                  marker: "fqa",
                  content: ["may now receive mercy through your mercy"],
                },
                "; BYZ and TR ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["may receive mercy through your mercy"],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "32" },
        "For God has consigned everyone to disobedience so that He may have mercy on everyone.",
      ],
    },
    { type: "para", marker: "s1", content: ["A Hymn of Praise"] },
    { type: "para", marker: "r", content: ["(Isaiah 40:9–31)"] },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "33" },
        "O, the depth of the riches",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: ["of the wisdom and knowledge of God!"],
    },
    {
      type: "para",
      marker: "q1",
      content: ["How unsearchable are His judgments,"],
    },
    { type: "para", marker: "q2", content: ["and untraceable His ways!"] },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "34" },
        "“Who has known the mind of the Lord?",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "Or who has been His counselor?”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["11:34"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 40:13"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "35" },
        "“Who has first given to God,",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "that God should repay him?”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["11:35"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Job 41:11"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        { type: "verse", marker: "v", number: "36" },
        "For from Him and through Him and to Him are all things.",
      ],
    },
    {
      type: "para",
      marker: "q2",
      content: ["To Him be the glory forever! Amen."],
    },
    { type: "chapter", marker: "c", number: "12" },
    { type: "para", marker: "s1", content: ["Living Sacrifices"] },
    {
      type: "para",
      marker: "r",
      content: ["(1 Corinthians 3:16–23; 1 Corinthians 6:18–20)"],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "Therefore I urge you, brothers, on account of God’s mercy, to offer your bodies as living sacrifices, holy and pleasing to God, which is your spiritual service of worship.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["12:1"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["your reasonable service"],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "2" },
        "Do not be conformed to this world, but be transformed by the renewing of your mind. Then you will be able to test and approve what is the good, pleasing, and perfect will of God.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "3" },
        "For by the grace given me I say to every one of you: Do not think of yourself more highly than you ought, but think of yourself with sober judgment, according to the measure of faith God has given you.",
        { type: "verse", marker: "v", number: "4" },
        "Just as each of us has one body with many members, and not all members have the same function,",
        { type: "verse", marker: "v", number: "5" },
        "so in Christ we who are many are one body, and each member belongs to one another.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "6" },
        "We have different gifts according to the grace given us. If one’s gift is prophecy, let him use it in proportion to his faith;",
        { type: "verse", marker: "v", number: "7" },
        "if it is serving, let him serve; if it is teaching, let him teach;",
        { type: "verse", marker: "v", number: "8" },
        "if it is encouraging, let him encourage; if it is giving, let him give generously; if it is leading, let him lead with diligence; if it is showing mercy, let him do it cheerfully.",
      ],
    },
    { type: "para", marker: "s1", content: ["Love, Zeal, Hope, Hospitality"] },
    { type: "para", marker: "r", content: ["(John 13:31–35; 1 John 3:11–24)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "9" },
        "Love must be sincere. Detest what is evil; cling to what is good.",
        { type: "verse", marker: "v", number: "10" },
        "Be devoted to one another in brotherly love. Outdo yourselves in honoring one another.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "11" },
        "Do not let your zeal subside; keep your spiritual fervor, serving the Lord.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "12" },
        "Be joyful in hope, patient in affliction, persistent in prayer.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "13" },
        "Share with the saints who are in need. Practice hospitality.",
      ],
    },
    { type: "para", marker: "s1", content: ["Forgiveness"] },
    { type: "para", marker: "r", content: ["(Matthew 18:21–35)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "14" },
        "Bless those who persecute you. Bless and do not curse.",
        { type: "verse", marker: "v", number: "15" },
        "Rejoice with those who rejoice; weep with those who weep.",
        { type: "verse", marker: "v", number: "16" },
        "Live in harmony with one another. Do not be proud, but associate with the lowly. Do not be conceited.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "17" },
        "Do not repay anyone evil for evil. Carefully consider what is right in the eyes of everybody.",
        { type: "verse", marker: "v", number: "18" },
        "If it is possible on your part, live at peace with everyone.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "19" },
        "Do not avenge yourselves, beloved, but leave room for God’s wrath. For it is written: “Vengeance is Mine; I will repay, says the Lord.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["12:19"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Deuteronomy 32:35"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "20" },
        "On the contrary,",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“If your enemy is hungry, feed him;"],
    },
    {
      type: "para",
      marker: "q2",
      content: ["if he is thirsty, give him a drink."],
    },
    { type: "para", marker: "q1", content: ["For in so doing,"] },
    {
      type: "para",
      marker: "q2",
      content: [
        "you will heap burning coals on his head.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["12:20"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Proverbs 25:21–22"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "21" },
        "Do not be overcome by evil, but overcome evil with good.",
      ],
    },
    { type: "chapter", marker: "c", number: "13" },
    { type: "para", marker: "s1", content: ["Submission to Authorities"] },
    { type: "para", marker: "r", content: ["(1 Peter 2:13–20)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "Everyone must submit himself to the governing authorities, for there is no authority except that which is from God. The authorities that exist have been appointed by God.",
        { type: "verse", marker: "v", number: "2" },
        "Consequently, whoever resists authority is opposing what God has set in place, and those who do so will bring judgment on themselves.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "3" },
        "For rulers are not a terror to good conduct, but to bad. Do you want to be unafraid of the one in authority? Then do what is right, and you will have his approval.",
        { type: "verse", marker: "v", number: "4" },
        "For he is God’s servant for your good. But if you do wrong, be afraid, for he does not carry the sword in vain. He is God’s servant, an agent of retribution to the wrongdoer.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "5" },
        "Therefore it is necessary to submit to authority, not only to avoid punishment, but also as a matter of conscience.",
        { type: "verse", marker: "v", number: "6" },
        "This is also why you pay taxes. For the authorities are God’s servants, who devote themselves to their work.",
        { type: "verse", marker: "v", number: "7" },
        "Pay everyone what you owe him: taxes to whom taxes are due, revenue to whom revenue is due, respect to whom respect is due, honor to whom honor is due.",
      ],
    },
    { type: "para", marker: "s1", content: ["Love Fulfills the Law"] },
    { type: "para", marker: "r", content: ["(Leviticus 19:9–18)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "8" },
        "Be indebted to no one, except to one another in love. For he who loves his neighbor has fulfilled the law.",
        { type: "verse", marker: "v", number: "9" },
        "The commandments “Do not commit adultery,” “Do not murder,” “Do not steal,” “Do not covet,”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["13:9"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Exodus 20:13–17"] },
                ";",
                {
                  type: "char",
                  marker: "xt",
                  content: ["Deuteronomy 5:17–21"],
                },
              ],
            },
          ],
          caller: "+",
        },
        "and any other commandments, are summed up in this one decree: “Love your neighbor as yourself.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["13:9"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Leviticus 19:18"] },
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "10" },
        "Love does no wrong to its neighbor. Therefore love is the fulfillment of the law.",
      ],
    },
    { type: "para", marker: "s1", content: ["The Day Is Near"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "11" },
        "And do this, understanding the occasion. The hour has come for you to wake up from your slumber, for our salvation is nearer now than when we first believed.",
        { type: "verse", marker: "v", number: "12" },
        "The night is nearly over; the day has drawn near. So let us lay aside the deeds of darkness and put on the armor of light.",
        { type: "verse", marker: "v", number: "13" },
        "Let us behave decently, as in the daytime, not in carousing and drunkenness, not in sexual immorality and debauchery, not in dissension and jealousy.",
        { type: "verse", marker: "v", number: "14" },
        "Instead, clothe yourselves with",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["13:14"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                { type: "char", marker: "fq", content: ["put on"] },
                "; see verse 12.",
              ],
            },
          ],
          caller: "+",
        },
        "the Lord Jesus Christ, and make no provision for the desires of the flesh.",
      ],
    },
    { type: "chapter", marker: "c", number: "14" },
    { type: "para", marker: "s1", content: ["The Law of Liberty"] },
    { type: "para", marker: "r", content: ["(Matthew 7:1–6; Luke 6:37–42)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "Accept him whose faith is weak, without passing judgment on his opinions.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["14:1"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["without quarreling over disputable matters"],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "2" },
        "For one person has faith to eat all things, while another, who is weak, eats only vegetables.",
        { type: "verse", marker: "v", number: "3" },
        "The one who eats everything must not belittle the one who does not, and the one who does not eat everything must not judge the one who does, for God has accepted him.",
        { type: "verse", marker: "v", number: "4" },
        "Who are you to judge someone else’s servant? To his own master he stands or falls. And he will stand, for the Lord is able to make him stand.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "5" },
        "One person regards a certain day above the others, while someone else considers every day alike. Each one should be fully convinced in his own mind.",
        { type: "verse", marker: "v", number: "6" },
        "He who observes a special day does so to the Lord;",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["14:6"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "BYZ and TR include ",
                {
                  type: "char",
                  marker: "fq",
                  content: [
                    "he who does not regard the day, to the Lord he does not regard it;",
                  ],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "he who eats does so to the Lord, for he gives thanks to God; and he who abstains does so to the Lord and gives thanks to God.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "7" },
        "For none of us lives to himself alone, and none of us dies to himself alone.",
        { type: "verse", marker: "v", number: "8" },
        "If we live, we live to the Lord, and if we die, we die to the Lord. So whether we live or die, we belong to the Lord.",
        { type: "verse", marker: "v", number: "9" },
        "For this reason Christ died and returned to life, that He might be the Lord of both the dead and the living.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "10" },
        "Why, then, do you judge your brother? Or why do you belittle your brother? For we will all stand before God’s judgment seat.",
        { type: "verse", marker: "v", number: "11" },
        "It is written:",
      ],
    },
    { type: "para", marker: "q1", content: ["“As surely as I live,"] },
    { type: "para", marker: "q2", content: ["says the Lord,"] },
    { type: "para", marker: "q1", content: ["every knee will bow before Me;"] },
    {
      type: "para",
      marker: "q2",
      content: [
        "every tongue will confess to God.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["14:11"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["will give praise to God"],
                },
                " or ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["will acknowledge God"],
                },
                ";",
                { type: "char", marker: "xt", content: ["Isaiah 45:23"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "12" },
        "So then, each of us will give an account of himself to God.",
      ],
    },
    { type: "para", marker: "s1", content: ["The Law of Love"] },
    {
      type: "para",
      marker: "r",
      content: ["(Ezekiel 14:1–11; 1 Corinthians 8:1–13)"],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "13" },
        "Therefore let us stop judging one another. Instead, make up your mind not to put any stumbling block or obstacle in your brother’s way.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "14" },
        "I am convinced and fully persuaded in the Lord Jesus that nothing is unclean in itself. But if anyone regards something as unclean, then for him it is unclean.",
        { type: "verse", marker: "v", number: "15" },
        "If your brother is distressed by what you eat, you are no longer acting in love. Do not by your eating destroy your brother, for whom Christ died.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "16" },
        "Do not allow what you consider good, then, to be spoken of as evil.",
        { type: "verse", marker: "v", number: "17" },
        "For the kingdom of God is not a matter of eating and drinking, but of righteousness, peace, and joy in the Holy Spirit.",
        { type: "verse", marker: "v", number: "18" },
        "For whoever serves Christ in this way is pleasing to God and approved by men.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "19" },
        "So then, let us pursue what leads to peace and to mutual edification.",
        { type: "verse", marker: "v", number: "20" },
        "Do not destroy the work of God for the sake of food. All food is clean, but it is wrong for a man to let his eating be a stumbling block.",
        { type: "verse", marker: "v", number: "21" },
        "It is better not to eat meat or drink wine or to do anything to cause your brother to stumble.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["14:21"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "SBL, BYZ, and TR include ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["or to be hindered or weakened"],
                },
                ".",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "22" },
        "Keep your belief about such matters between yourself and God.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["14:22"] },
            {
              type: "char",
              marker: "ft",
              content: [
                {
                  type: "char",
                  marker: "fqa",
                  content: [
                    "Keep the faith that you have to yourself before God",
                  ],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "Blessed is the one who does not condemn himself by what he approves.",
        { type: "verse", marker: "v", number: "23" },
        "But the one who has doubts is condemned if he eats, because his eating is not from faith; and everything that is not from faith is sin.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["14:23"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Some manuscripts place the text of",
                { type: "char", marker: "xt", content: ["Romans 16:25–27"] },
                "here.",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    { type: "chapter", marker: "c", number: "15" },
    { type: "para", marker: "s1", content: ["Accept One Another"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "We who are strong ought to bear with the shortcomings of the weak and not to please ourselves.",
        { type: "verse", marker: "v", number: "2" },
        "Each of us should please his neighbor for his good, to build him up.",
        { type: "verse", marker: "v", number: "3" },
        "For even Christ did not please Himself, but as it is written: “The insults of those who insult You have fallen on Me.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["15:3"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Psalm 69:9"] },
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "4" },
        "For everything that was written in the past was written for our instruction, so that through endurance and the encouragement of the Scriptures, we might have hope.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "5" },
        "Now may the God who gives endurance and encouragement grant you harmony with one another in Christ Jesus,",
        { type: "verse", marker: "v", number: "6" },
        "so that with one mind and one voice you may glorify the God and Father of our Lord Jesus Christ.",
      ],
    },
    {
      type: "para",
      marker: "s1",
      content: ["Christ the Servant of Jews and Gentiles"],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "7" },
        "Accept one another, then, just as Christ accepted you, in order to bring glory to God.",
        { type: "verse", marker: "v", number: "8" },
        "For I tell you that Christ has become a servant of the circumcised on behalf of God’s truth, to confirm the promises made to the patriarchs,",
        { type: "verse", marker: "v", number: "9" },
        "so that the Gentiles may glorify God for His mercy. As it is written:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“Therefore I will praise You among the Gentiles;"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "I will sing hymns to Your name.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["15:9"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["2 Samuel 22:50"] },
                ";",
                { type: "char", marker: "xt", content: ["Psalm 18:49"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "10" },
        "Again, it says:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: [
        "“Rejoice, O Gentiles, with His people.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["15:10"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Deuteronomy 32:43"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [{ type: "verse", marker: "v", number: "11" }, "And again:"],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“Praise the Lord, all you Gentiles,"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "and extol Him, all you peoples.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["15:11"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Psalm 117:1"] },
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "12" },
        "And once more, Isaiah says:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“The Root of Jesse will appear,"],
    },
    {
      type: "para",
      marker: "q2",
      content: ["One who will arise to rule over the Gentiles;"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "in Him the Gentiles will put their hope.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["15:12"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 11:10"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "13" },
        "Now may the God of hope fill you with all joy and peace as you believe in Him, so that you may overflow with hope by the power of the Holy Spirit.",
      ],
    },
    {
      type: "para",
      marker: "s1",
      content: ["Paul the Minister to the Gentiles"],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "14" },
        "I myself am convinced, my brothers, that you yourselves are full of goodness, brimming with knowledge, and able to instruct one another.",
        { type: "verse", marker: "v", number: "15" },
        "However, I have written you a bold reminder on some points, because of the grace God has given me",
        { type: "verse", marker: "v", number: "16" },
        "to be a minister of Christ Jesus to the Gentiles in the priestly service of the gospel of God, so that the Gentiles might become an offering acceptable to God, sanctified by the Holy Spirit.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "17" },
        "Therefore I exult in Christ Jesus in my service to God.",
        { type: "verse", marker: "v", number: "18" },
        "I will not presume to speak of anything except what Christ has accomplished through me in leading the Gentiles to obedience by word and deed,",
        { type: "verse", marker: "v", number: "19" },
        "by the power of signs and wonders, and by the power of the Spirit of God.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["15:19"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "SBL ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["the power of the Spirit"],
                },
                "; NE and WH ",
                {
                  type: "char",
                  marker: "fq",
                  content: ["the power of the Holy Spirit"],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "So from Jerusalem all the way around to Illyricum, I have fully proclaimed the gospel of Christ.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "20" },
        "In this way I have aspired to preach the gospel where Christ was not known, so that I would not be building on someone else’s foundation.",
        { type: "verse", marker: "v", number: "21" },
        "Rather, as it is written:",
      ],
    },
    {
      type: "para",
      marker: "q1",
      content: ["“Those who were not told about Him will see,"],
    },
    {
      type: "para",
      marker: "q2",
      content: [
        "and those who have not heard will understand.”",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["15:21"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "",
                { type: "char", marker: "xt", content: ["Isaiah 52:15"] },
                "(see also LXX)",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "22" },
        "That is why I have often been hindered from coming to you.",
      ],
    },
    { type: "para", marker: "s1", content: ["Paul’s Travel Plans"] },
    { type: "para", marker: "r", content: ["(1 Corinthians 16:5–9)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "23" },
        "But now that there are no further opportunities for me in these regions, and since I have longed for many years to visit you,",
        { type: "verse", marker: "v", number: "24" },
        "I hope to see you on my way to Spain. And after I have enjoyed your company for a while, you can equip me for my journey.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "25" },
        "Now, however, I am on my way to Jerusalem to serve the saints there.",
        { type: "verse", marker: "v", number: "26" },
        "For Macedonia and Achaia were pleased to make a contribution for the poor among the saints in Jerusalem.",
        { type: "verse", marker: "v", number: "27" },
        "They were pleased to do it, and indeed they owe it to them. For if the Gentiles have shared in their spiritual blessings, they are obligated to minister to them with material blessings.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "28" },
        "So after I have completed this service and have safely delivered this bounty to them, I will set off to Spain by way of you.",
        { type: "verse", marker: "v", number: "29" },
        "I know that when I come to you, I will come in the fullness of the blessing",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["15:29"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "TR and BYZ include ",
                { type: "char", marker: "fq", content: ["of the gospel"] },
                ".",
              ],
            },
          ],
          caller: "+",
        },
        "of Christ.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "30" },
        "Now I urge you, brothers, by our Lord Jesus Christ and by the love of the Spirit, to join me in my struggle by praying to God for me.",
        { type: "verse", marker: "v", number: "31" },
        "Pray that I may be delivered from the unbelievers in Judea, and that my service in Jerusalem may be acceptable to the saints there,",
        { type: "verse", marker: "v", number: "32" },
        "so that by God’s will I may come to you with joy and together with you be refreshed.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "33" },
        "The God of peace be with all of you. Amen.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["15:33"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Some manuscripts do not include ",
                { type: "char", marker: "fq", content: ["Amen."] },
                " One early manuscript places the text of",
                { type: "char", marker: "xt", content: ["Romans 16:25–27"] },
                "here.",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    { type: "chapter", marker: "c", number: "16" },
    { type: "para", marker: "s1", content: ["Personal Greetings and Love"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "1" },
        "I commend to you our sister Phoebe, a servant",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["16:1"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Or ",
                { type: "char", marker: "fq", content: ["deaconess"] },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "of the church in Cenchrea.",
        { type: "verse", marker: "v", number: "2" },
        "Welcome her in the Lord in a manner worthy of the saints, and assist her with anything she may need from you. For she has been a great help to many people, including me.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "3" },
        "Greet Prisca",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["16:3"] },
            {
              type: "char",
              marker: "ft",
              content: [
                { type: "char", marker: "fq", content: ["Prisca"] },
                " is a variant of ",
                { type: "char", marker: "fq", content: ["Priscilla"] },
                "; see",
                { type: "char", marker: "xt", content: ["Acts 18:2"] },
                ".",
              ],
            },
          ],
          caller: "+",
        },
        "and Aquila, my fellow workers in Christ Jesus,",
        { type: "verse", marker: "v", number: "4" },
        "who have risked their lives for me. Not only I but all the churches of the Gentiles are grateful to them.",
        { type: "verse", marker: "v", number: "5" },
        "Greet also the church that meets at their house.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        "Greet my beloved Epenetus, who was the first convert to Christ in the province of Asia.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["16:5"] },
            {
              type: "char",
              marker: "ft",
              content: [
                { type: "char", marker: "fqa", content: ["in Asia"] },
                "; Asia was a Roman province in what is now western Turkey.",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "6" },
        "Greet Mary, who has worked very hard for you.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "7" },
        "Greet Andronicus and Junia,",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["16:7"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Some translators ",
                { type: "char", marker: "fq", content: ["Junias"] },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "my fellow countrymen and fellow prisoners. They are outstanding among the apostles, and they were in Christ before I was.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "8" },
        "Greet Ampliatus, my beloved in the Lord.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "9" },
        "Greet Urbanus, our fellow worker in Christ, and my beloved Stachys.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "10" },
        "Greet Apelles, who is approved in Christ.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: ["Greet those who belong to the household of Aristobulus."],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "11" },
        "Greet Herodion, my fellow countryman.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        "Greet those from the household of Narcissus who are in the Lord.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "12" },
        "Greet Tryphena and Tryphosa, women who have worked hard in the Lord.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        "Greet my beloved Persis, who has worked very hard in the Lord.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "13" },
        "Greet Rufus, chosen in the Lord, and his mother, who has been a mother to me as well.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "14" },
        "Greet Asyncritus, Phlegon, Hermes, Patrobas, Hermas, and the brothers with them.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "15" },
        "Greet Philologus and Julia, Nereus and his sister, and Olympas and all the saints with them.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "16" },
        "Greet one another with a holy kiss.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: ["All the churches of Christ send you greetings."],
    },
    { type: "para", marker: "s1", content: ["Avoid Divisions"] },
    { type: "para", marker: "r", content: ["(Titus 3:9–11)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "17" },
        "Now I urge you, brothers, to watch out for those who create divisions and obstacles that are contrary to the teaching you have learned. Turn away from them.",
        { type: "verse", marker: "v", number: "18" },
        "For such people are not serving our Lord Christ, but their own appetites. By smooth talk and flattery they deceive the hearts of the naive.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "19" },
        "Everyone has heard about your obedience, so I rejoice over you. But I want you to be wise about what is good and innocent about what is evil.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "20" },
        "The God of peace will soon crush Satan under your feet. The grace of our Lord Jesus Christ",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["16:20"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "NA, NE, and WH ",
                { type: "char", marker: "fq", content: ["Lord Jesus"] },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "be with you.",
      ],
    },
    {
      type: "para",
      marker: "s1",
      content: ["Greetings from Paul’s Fellow Workers"],
    },
    { type: "para", marker: "r", content: ["(Colossians 4:7–14)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "21" },
        "Timothy, my fellow worker, sends you greetings, as do Lucius, Jason, and Sosipater, my fellow countrymen.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "22" },
        "I, Tertius, who wrote down this letter, greet you in the Lord.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "23" },
        "Gaius, who has hosted me and all the church, sends you greetings.",
      ],
    },
    {
      type: "para",
      marker: "p",
      content: [
        "Erastus, the city treasurer, sends you greetings, as does our brother Quartus.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["16:23"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "SBL, BYZ, and TR include ",
                {
                  type: "char",
                  marker: "fq",
                  content: [
                    "24May the grace of our Lord Jesus Christ be with you all. Amen.",
                  ],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        { type: "verse", marker: "v", number: "24" },
      ],
    },
    { type: "para", marker: "s1", content: ["Doxology"] },
    { type: "para", marker: "r", content: ["(Jude 1:24–25)"] },
    {
      type: "para",
      marker: "p",
      content: [
        { type: "verse", marker: "v", number: "25" },
        "Now to Him who is able to strengthen you by my gospel and by the proclamation of Jesus Christ, according to the revelation of the mystery concealed for ages past",
        { type: "verse", marker: "v", number: "26" },
        "but now revealed and made known through the writings of the prophets by the command of the eternal God, in order to lead all nations to the obedience that comes from faith",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["16:26"] },
            {
              type: "char",
              marker: "ft",
              content: [
                {
                  type: "char",
                  marker: "fqa",
                  content: ["the obedience of faith"],
                },
                "",
              ],
            },
          ],
          caller: "+",
        },
        "—",
        { type: "verse", marker: "v", number: "27" },
        "to the only wise God be glory forever through Jesus Christ! Amen.",
        {
          type: "note",
          marker: "f",
          content: [
            { type: "char", marker: "fr", content: ["16:27"] },
            {
              type: "char",
              marker: "ft",
              content: [
                "Some manuscripts place the text of verses 25–27 after",
                { type: "char", marker: "xt", content: ["Romans 14:23"] },
                "or after",
                { type: "char", marker: "xt", content: ["Romans 15:33"] },
                ".",
              ],
            },
          ],
          caller: "+",
        },
      ],
    },
  ],
};
